...
local AddPrefabPostInit, pairs, GetModConfigData, math_floor, ipairs, print = AddPrefabPostInit, pairs, GetModConfigData, math.floor, ipairs, print
...
inst:ListenForEvent("timerdone", function()
TheNet:Announce(annstring.hoodedwidow.. " " ..annstring.reflash)
end)
end)
...
--days in advance config
local HOUND_DAYS_IN_ADVANCE = GetModConfigData('hound_in_advance')
local WORM_DAYS_IN_ADVANCE = GetModConfigData('worm_in_advance') 
local secADay = 8*60
...
local function HoundAttack(inst)
	...
	function(inst)
		...
		if timeToAttack <= HOUND_DAYS_IN_ADVANCE and GLOBAL.TheWorld.state.cycles ~= 0 then
	...
	local function WormAttack(inst)
		...
		function(inst)
			...
			local _timeToAttack = GLOBAL.TheWorld.components.hounded:GetTimeToAttack()
			local timeToAttack  = second2Day(_timeToAttack)
			if timeToAttack <= WORM_DAYS_IN_ADVANCE and GLOBAL.TheWorld.state.cycles ~= 0 then
